...
def read_structure_from_string(source, format, id_tag=None, errors="strict"):
    """read_structure_from_string(string, format) -> (id, ObMol)

    Read single structure from string and returning a pair of 
    title and OBMol. 
    """
    ...
    success = obconversion.ReadString(obmol, str(source))
    errmsg = None
    if HAS_ERROR_LOG:
        ob.obErrorLog.SetOutputLevel(lvl) # Restore message level
        if ob.obErrorLog.GetErrorMessageCount():
...
def _read_structure_from_string(metadata, source, format, id_tag=None, errors="strict"):
    if metadata.aromaticity is not None:
        raise ValueError("Open Babel does not support alternate aromaticity models "
                         "(want aromaticity=%r)" % metadata.aromaticity)
    return read_structure_from_string(source, format, id_tag, errors)

_base = FingerprintFamilyConfig(
    software=SOFTWARE,
    read_structures=_read_structures,
    read_structure_from_string=_read_structure_from_string,
    )
